// AffinityUnhide.cpp - DLL to set WDA_EXCLUDEFROMCAPTURE
#include <windows.h>
#include <string>
#include <shlwapi.h>
#include "pch.h"
...
void ApplyNormalAffinity() {
    ...
    DebugLog("====== Setting WDA_EXCLUDEFROMCAPTURE for %s ======", procName.c_str());
    ...
    while ((hwnd = FindWindowEx(NULL, hwnd, NULL, NULL)) != NULL) {
        ...
        if (pid == GetCurrentProcessId() && IsWindowVisible(hwnd)) {
            ...
            if (SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)) {
                DebugLog("[SUCCESS] Window: %s - Set to WDA_EXCLUDEFROMCAPTURE", (title[0] ? title : "<No Title>"));
                windowCount++;
            }
            else {
